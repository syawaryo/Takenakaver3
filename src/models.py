...
class OcrParagraph(BaseModel):
    """DI paragraphレベルのテキストブロック"""
    content: str = Field(..., description="paragraph全体のテキスト")
    bbox: BBox = Field(..., description="paragraphのバウンディングボックス(px)")
    center_px: PixelPoint = Field(..., description="paragraph中心座標(px)")
